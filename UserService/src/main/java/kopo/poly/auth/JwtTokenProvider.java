...
public class JwtTokenProvider {
    ...
    public String createToken(String userId, String roles, JwtTokenType tokenType) {
        ...
        if (tokenType == JwtTokenType.ACCESS_TOKEN) {   // Access Token 이라면
        ...
        Claims claims = Jwts.claims()
                ...
                .setSubject(userId);    // 회원 아이디 저장 : PK 저장(userId)
    ...
    public TokenDTO getTokenInfo(String token) {
        ...
        String role = CmmUtil.nvl((String) claims.get("roles"));    // LoginService 에서 생성된 토큰의 권한명과 동일

        log.info("userId : " + userId);
        log.info("role : " + role);

        // TokenDTO 는 자바 17의 Record 객체 사용했기에 빌더패턴 적용함
    ...
    /**
     * 쿠키 및 인증헤더(Bearer)에 저장된 JWT 토큰(Access Token, Refresh Token) 가져오기
     *
    ...
    public String resolveToken(HttpServletRequest request, JwtTokenType tokenType) {
        ...
        // Cookie 에 저장된 데이터 모두 가져오기
        Cookie[] cookies = request.getCookies();

        if (cookies != null) {  // Cookie 가 존재하면, Cookie 에서 토큰 값 가져오기
        ...
        // Cookie 에 토큰이 존재하지 않으면, Bearer 토큰에 값이 있는지 확인함
        if (token.length() == 0) {
